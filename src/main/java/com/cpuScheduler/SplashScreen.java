...
public class SplashScreen extends JWindow {
    ...
    public SplashScreen(Runnable onFinished) {
        ...
        // Use sound duration if available, otherwise fall back to 3 s
        long displayMs = soundDurationMs > 0 ? soundDurationMs : 3000;

        // ---- Close splash after sound ends ----
    ...
    private long playSplashSound() {
        try {
            ...
            if (splashClip.isControlSupported(FloatControl.Type.MASTER_GAIN)) {
                FloatControl gain = (FloatControl) splashClip.getControl(FloatControl.Type.MASTER_GAIN);
                gain.setValue(Math.max(gain.getMinimum(), -3.0f));
            }
